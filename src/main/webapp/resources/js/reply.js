...
var replyService = (function() {
	...
	function getList(param, callback, error) {
		...
		$.getJSON("/jcw/comment/pages/" + board_idx + "/" + page + ".json", function(data) {
			if (callback) {
	...
	// 댓글 삭제. security 적용 후
	function remove(idx, writer, callback, error) {
		
		console.log(JSON.stringify({writer:writer})); 
		console.log("--------------------------------------");  
		console.log(JSON.stringify({idx:idx, writer:writer}));  
		    
		$.ajax({
			type : 'delete',
			url : '/jcw/comment/delete/'+idx,
			data:  JSON.stringify({idx:idx,writer:writer}),
			contentType: "application/json; charset=utf-8",
	...
	function update(comment, callback, error) {
		...
		console.log("RNO: " + comment.idx);

		$.ajax({
			type : 'put',
			url : '/jcw/comment/modify/' + comment.idx,
	...
	function get(rno, callback, error) {
		...
		$.get("/jcw/comment/get/" + rno + ".json", function(result) {
			if (callback) {
				callback(result);
			}
		}).fail(function(xhr, status, err) {
